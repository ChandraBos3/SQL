...
cast(null as varchar(500)) itemdesc,HWOrq_Currency_Code, cast( null as varchar(15))currencydesc, DWOrq_Qty, DWORQ_Item_Rate, DWORQ_Item_Value,dworq_uom_code,
HWOrq_WOT_Code,MWOTP_Description, HWOrq_BA_Code,vendor_description, CAST( NULL AS VARCHAR(15)) NEWOLDCODE, cast (Null as date) code_Inserted_On, location, c.SBG_Code, HWOrq_Amount, HWOrq_Date,HWORQ_Inserted_On,HWOrq_DS_Code, DWORQ_Markup_Code, dworq_version
into #billlist
...
WHERE HWOrq_request_Number = DWOrq_request_Number
and HWORQ_Inserted_On>='01-Apr-2018' and HWOrq_DATE <='28-Aug-2018' 
...
--and HWOrq_request_Number = 'EF711wor8000046'
--and len (dworq_item_code) >= '9'
...
update a set city =UCITY_Name, state1=USTAT_Name
 ...
 and HWOrq_Job_Code =mjob_job_code
...
where MMKUP_Item_Group_Code = left(dworq_item_code,4)
and MMKUP_MC_Code = c.MMKCT_MC_Code
AND MMKUP_Markup_Code=DWORQ_MARKUP_CODE
AND B.MMKUP_Job_Code = a.HWOrq_Job_Code
...
where TEPWI_Job_Code = a.HWOrq_Job_Code
and TEPWI_Work_Item_Code = a.DWORQ_Item_Code
and tepwi_markup= dworq_markup_code
...
UPDATE A SET a.Constzon_Applicable  = b.DWORC_Rate_Applicable
from #billlist a
left join eip.SQLWOM.WOM_D_WO_Request_ConstZon_Details b  on 
 hWORQ_Request_Number = DWORC_Request_Number
					AND DWORQ_Item_Code = DWORC_Item_Code AND DWORQ_Version = DWORC_Version AND DWORQ_Markup_Code = DWORC_Markup_Code

alter table #billlist add planning_inserted_on  Date

Update a set planning_inserted_on = TEPWI_Inserted_on
from #billlist a
left join epm.SQLPMP.PMP_T_ExecPlan_Project_Work_Items b on dworq_item_code= TEPWI_Work_Item_Code and DWORQ_Markup_Code= TEPWI_MarkUp and a.HWOrq_Job_Code=TEPWI_Job_Code

alter table #billlist add EPM_Tag varchar (5)

Update a set EPM_Tag = TCM_EPM_Tag 
from #billlist a 

left join EPM.sqlepm.EPM_M_Control_Master b on HWORQ_JOB_CODE=TCM_Job_Code 



SELECT *FROM #billlist where len(dworq_item_code) ='9' and newoldcode ='new'



USE EIP
GO
SELECT *FROM eip.SQLWOM.WOM_D_WO_Request_ConstZon_Details
