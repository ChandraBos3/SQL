...
@uid=525, @to_date = '30-jun-2017', @NoofDays = 30 --,@warehouse_code='C04861', 
...
-- Exec accesscontrol.dbo.lntsp_security_access_list @Uid = @uid, @role_id='rgecc', @cluster_desc='Job', @cluster_list=3 
 ...
 Declare @New_Warehouse_Code int     
  ...
  Surplus_qty Money)   
   ...
   FROM live.eip.SQLSCM.SCM_T_Warehouse_Stock wo  
     inner Join live.eip.SQLMAS.GEN_M_Materials mm on (wo.TWHS_Material_Code = mm.MMAT_Material_Code and TWHS_Company_Code=MMAT_Company_Code)  
     inner Join live.eip.SQLMAS.GEN_U_Unit_Of_Measurement  um on (mm.MMAT_UOM_Code = um.UUOM_UOM_Code)  
     Inner JOIN live.eip.SQLMAS.GEN_M_Jobs  jm on (MJOB_Job_Code = wo.TWHS_Job_Code)
	 Inner JOIN live.eip.SQLMAS.GEN_L_Job_Cluster_Elements  ON (LJCE_Job_Code = wo.TWHS_Job_Code  )
	 Inner JOIN live.eip.SQLMAS.GEN_M_Cluster_Element_Details IC  ON (IC.MCLED_CED_Code=LJCE_IC_Code 
																	AND IC.MCLED_CE_Code=2 
																	AND IC.MCLED_Company_Code = @company_code )
	 Inner JOIN live.eip.SQLMAS.GEN_M_Cluster_Element_Details BU ON (BU.MCLED_CED_Code=LJCE_bu_Code AND BU.MCLED_CE_Code=4
																	AND BU.MCLED_Company_Code = @company_code)
	 Inner JOIN live.eip.SQLMAS.GEN_M_Cluster_Element_Details Cluster ON (Cluster.MCLED_CED_Code= LJCE_Cluster_Office_Code 
 ...
 -- print @company_code
  /*Issue*/	
		...
		From live.eip.sqlscm.SCM_H_Issue,
			 live.eip.SQLSCM.SCM_D_Issue
	...
	/*MRN*/
		...
		FROM live.eip.SQLSCM.SCM_H_MRN
		INNER JOIN live.eip.SQLSCM.SCM_D_MRN ON (HMRN_MRN_Number = DMRN_MRN_Number 
	...
	/*DC*/
		...
		from live.eip.SQLSCM.SCM_H_DC,
			live.eip.SQLSCM.SCM_D_DC
	...
	/*IJSTN*/
		...
		FROM live.eip.sqlscm.SCM_H_Inter_Job_Stock_Transfer_Note,
			 live.eip.SQLSCM.SCM_D_Inter_Job_Stock_Transfer_Note
	...
	/*Master Data*/
		...
		from live.EIP.SQLmas.GEN_M_Transaction_Details
		...
		from live.eip.SQLmas.GEN_L_Job_Cluster_Elements,
		...
		from live.eip. SQLmas.GEN_L_Job_Cluster_Elements,
		live.eip.SQLMAS.GEN_M_Cluster_Element_Details
		where job_code = LJCE_Job_Code
		...
		from live.eip.SQLMAS.GEN_L_Job_Currency,
		live.eip.SQLMAS.GEN_M_Currencies
		...
		from live.eip.SQLSCM.SCM_T_Surplus_Stock
		where 
 ...
 SELECT  tos.stock_type+' - '+ tos.stock_Description stock_type,  
  ...
  select *from sys.tables where name like '%ADD%'

  select *from eip.SQLMAS.GEN_M_Warehouses
  select *from eip.sqlmas.gen_l_job_warehouses where ljwh_company_code = '1' and LJWH_IsActive = 'Y'
  
  select *from eip.SQLMAS.GEN_M_Company_Warehouse_Locations
    select *from eip.SQLMAS.GEN_L_Warehouse_Locations
	    select *from eip.SQLMAS.GEN_M_Address_Book
		select *from eip.sqlmas.gen_m_


		drop table #temp1
  SELECT LJWH_JOB_CODE,B.JOB_DESCRIPTION, LJWH_WAREHOUSE_CODE, MWH_DESCRIPTION, B.SECTOR_CODE, B.BU_CODE,MWH_AB_Code 
  INTO #TEMP1
  FROM  eip.SQLMAS.GEN_M_Warehouses, eip.sqlmas.gen_l_job_warehouses, LNT.dbo.job_master b
  where ljwh_company_code = '1' and LJWH_IsActive = 'Y'
  AND LJWH_Warehouse_Code =MWH_Warehouse_Code
  AND LJWH_JOB_CODE = JOB_CODE
  AND LJWH_Company_Code=MWH_Company_Code
...
Update #temp1 set MWH_DESCRIPTION = replace(MWH_DESCRIPTION , char(10),'-')

Update #temp1 set MWH_DESCRIPTION = replace(MWH_DESCRIPTION , char(11),'-')

Update #temp1 set MWH_DESCRIPTION = replace(MWH_DESCRIPTION , char(12),'-')

Update #temp1 set MWH_DESCRIPTION = replace(MWH_DESCRIPTION , char(13),'-')

Update #temp1 set MWH_DESCRIPTION = replace(MWH_DESCRIPTION , char(14),'-')

Update #temp1 set MWH_DESCRIPTION=replace (MWH_DESCRIPTION , '''','f')
Update #temp1 set MWH_DESCRIPTION=replace (MWH_DESCRIPTION , '"','i')
